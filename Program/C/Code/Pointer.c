...
//	int* ptr1 = (int*)(&aa + 1);	                                 //&aa二维数组指针，类型为int*[2][5],+1后ptr1指向末端，类型为int*
//	int* ptr2 = (int*)(*(aa + 1));	                                 //aa为第一个一维数组的地址，+1为第二个一维数组的地址，解引用为第二个一维数组，即第二个一维数组的首元素地址
//	printf("%d,%d", *(ptr1 - 2), *(ptr2 - 2));              	    
//	return 0;
//}

//int main()
//{
//	char* a[] = { "work","at","alibaba" };	               //a为三个字符串的首字符地址的指针数组
//	char** pa = a;	                                      //pa为指针数组的指针，指向指针数组的首元素
//	pa++;	                                              //跨过一个char*即指向第二个元素
//	printf("%s\n", *pa);              	                  
//	return 0;
//}
...
//int main()
//{
//	char* c[] = { "ENTER","NEW","POINT","FIRST" };                         	//四个字符串的首字符的地址数组，即char*类型的字符指针数组
//	char** cp[] = { c + 3,c + 2,c + 1,c };                              	//char**[]类型的指针数组
//	char*** cpp = cp;                                                   	//char**类型的数组首元素指针
//	printf("%s\n", **++cpp);                                       
//	printf("%s\n", *-- * ++cpp + 3);                                   
//	printf("%s\n", *cpp[-2] + 3);                    
//	printf("%s\n", cpp[-1][-1] + 1);       
